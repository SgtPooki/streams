...
export interface TransformFunctionTypes {
  transform: TransformerTransformCallback<Uint8Array, Uint8Array>
  flush?: TransformerFlushCallback<Uint8Array>
}

export const getTransformFunctions = (offset: number | undefined, length: number | undefined, logger?: ComponentLogger): TransformFunctionTypes => {
  const log = logger?.forComponent('sgtpooki:streams:transform-slice')
  log?.trace('offset=%O, length=%O', offset, length)
  // default is noop
  ...
  log?.trace('offsetOnlyUseCase=%O, lengthOnlyUseCase=%O, offsetAndLengthUseCase=%O', offsetOnlyUseCase, lengthOnlyUseCase, offsetAndLengthUseCase)
